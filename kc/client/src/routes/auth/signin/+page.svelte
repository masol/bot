<script>
	...
	import LoginSchema from '$lib/schemas/signin.json';
	import HasherCtrl from '$lib/utils/vars/hashvar';

	import signin from '$lib/images/signin.jpg';

	const hashctrl = HasherCtrl.inst

	const loginVarCtrl = new VarCtrl({
		syncTpl: '/v1/auth/login',
		bWritable: true,
		varPath: 'me',
		initData: {
			me: {
	...
	const loginVar = loginVarCtrl.var;
	const usernameSetter = loginVarCtrl.getEvtTrans('me.username');
	// console.log("loginVar=",loginVar)

	const handleSubmit = loginVarCtrl.getSubmit();
</script>
...
<div class="bg-white dark:bg-gray-900">
	<div class="flex justify-center h-screen">
		<div
			...
			style="background-image: url({{img_800_800__signin_jpg}})"
		>
			...
			<div class="flex-1">
				...
				<div class="mt-8">
					<form on:submit={handleSubmit}>
						<Helper helperClass="mb-3" color={$loginVar.mValid('me')}>
							{#if $loginVar.getMeta('me', 'errmsg')}
								{$loginVar.getMeta('me', 'errmsg')}
							{:else if $loginVar.getMeta('me', 'bValid', false)}
								{#if $loginVar.isPending()}
						...
						<div>
							...
							<Input
								...
								name="username"
								color={$loginVar.mValid('me.username')}
								id="username"
							...
							<Helper class="mt-2" color={$loginVar.mValid('me.username')}>
								{#if $loginVar.getMeta('me.username', 'errmsg')}
									{$loginVar.getMeta('me.username', 'errmsg')}
								{:else if $loginVar.getMeta('me.username', 'bValid', false)}
						...
						<div class="mt-6">
							...
							<Input
								bind:value={$loginVar.data.me.password}
								name="password"
								color={$loginVar.mValid('me.password')}
								id="password"
								type="password"
								placeholder="******"
							>
							...
							<Helper class="mt-2" color={$loginVar.mValid('me.password')}
								><span class="font-medium">
									{#if $loginVar.getMeta('me.password', 'errmsg')}
										{$loginVar.getMeta('me.password', 'errmsg')}
									{:else if $loginVar.getMeta('me.password', 'bValid', false)}
										<span class="font-medium">Well done!</span> Some success messsage.
									{:else}
										<span class="font-medium">初始提示:</span> Some messsage.
									{/if}
						...
						<Checkbox class="mt-6 space-x-1" bind:checked={$loginVar.data.me.keep} name="keep"
							>记住我</Checkbox
						>
					...
					<p class="mt-6 text-sm text-center text-gray-400">
						还没有品研帐号? <a
							href="signup"
							class="text-blue-500 focus:outline-none focus:underline hover:underline">立即注册</a
						>。
					</p>
				</div>
			</div>
