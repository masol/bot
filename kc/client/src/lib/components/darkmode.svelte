<script lang="ts">
	...
	export let btnClass: string =
		...
		'hidden ripple-bg-gray-300 ext-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5 ml-3 z-50 md:block'
...
<button on:click={toggleTheme} aria-label="夜间模式" type="button" class={btnClass}>
	{#if dark}
...
{#if canHover}
<Tooltip placement="bottom" shadow>
	{#if dark}
		切换到白天模式
	{:else}
		切换到夜晚模式
